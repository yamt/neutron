...
class L2populationRpcCallBackTunnelMixin(L2populationRpcCallBackMixin):
    ...
    @abc.abstractmethod
    def update_info_for_arp_reply(self, action, lvid, mac_address, ip_address):
    ...
    def fdb_chg_ip_tun(self, context, fdb_entries, local_ip, local_vlan_map):
        ...
        for network_id, agent_ports in fdb_entries.items():
            ...
            for agent_ip, state in agent_ports.items():
                ...
                for mac, ip in after:
                    self.update_info_for_arp_reply('add', lvm.vlan, mac, ip)

                before = state.get('before')
                for mac, ip in before:
                    self.update_info_for_arp_reply('remove', lvm.vlan, mac, ip)
