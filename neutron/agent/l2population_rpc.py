...
class L2populationRpcCallBackTunnelMixin(L2populationRpcCallBackMixin):
    ...
    @abc.abstractmethod
    def ope_arp_respond_info(self, action, lvid, mac_address, ip_address):
        '''Operate the ARP respond information.

        Do operation of arp respond infomation for an action
        In ovs do adding or removing flow entry to edit an arp reply.

        :param action: an action to operate for arp respond infomation.
            add or remove
        :param lvid: id in local VLAN map of network's ARP entry.
    ...
    def fdb_chg_ip_tun(self, context, fdb_entries, local_ip, local_vlan_map):
        ...
        for network_id, agent_ports in fdb_entries.items():
            ...
            for agent_ip, state in agent_ports.items():
                ...
                for mac, ip in after:
                    self.ope_arp_respond_info('add', lvm.vlan, mac, ip)

                before = state.get('before')
                for mac, ip in before:
                    self.ope_arp_respond_info('remove', lvm.vlan, mac, ip)
