...
class TunnelTest(object):
    ...
    def setUp(self):
        ...
        self.ovs_bridges = {
            self.INT_BRIDGE: mock.create_autospec(
                self.br_int_cls('br-int', driver_args=self.driver_args)),
            self.TUN_BRIDGE: mock.create_autospec(
                self.br_tun_cls('br-tun', driver_args=self.driver_args)),
            self.MAP_TUN_BRIDGE: mock.create_autospec(
                self.br_phys_cls('br-phys', driver_args=self.driver_args)),
        }
    ...
    def _define_expected_calls(self, arp_responder=False):
        self.mock_int_bridge_cls_expected = [
            mock.call(self.INT_BRIDGE, driver_args=self.driver_args),
        ]
        self.mock_phys_bridge_cls_expected = [
            mock.call(self.MAP_TUN_BRIDGE, driver_args=self.driver_args),
        ]
        self.mock_tun_bridge_cls_expected = [
            mock.call(self.TUN_BRIDGE, driver_args=self.driver_args),
        ]
    ...
    def _build_agent(self, **kwargs):
        backend_info = (
            self.driver_args,
            self.mock_int_bridge_cls,
            self.mock_phys_bridge_cls,
            self.mock_tun_bridge_cls,
        )
        kwargs.setdefault('backend_info', backend_info)
