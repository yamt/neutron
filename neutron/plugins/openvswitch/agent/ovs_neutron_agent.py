...
class OVSNeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def __init__(self, backend_info, integ_br, tun_br, local_ip,
        ...
        :param backend_info: OpenFlow backend info
        ...
        self.int_br = self.br_int_cls(integ_br, driver_args=self.driver_args)
            ...
            if bridge not in ovs_bridges:
                LOG.error(_LE("Bridge %(bridge)s for physical network "
                          ...
                          {'physical_network': physical_network,
                           'bridge': bridge})
                sys.exit(1)
            br = self.br_phys_cls(bridge, driver_args=self.driver_args)
...
def main(backend_info):
    ...
    prepare_xen_compute()
    try:
        agent = OVSNeutronAgent(backend_info, **agent_config)
