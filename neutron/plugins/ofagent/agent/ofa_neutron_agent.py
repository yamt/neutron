...
class OFANeutronAgent(n_rpc.RpcCallback,
    ...
    def update_info_for_arp_reply(self, action, lvid, mac_address, ip_address):
        if action == 'add':
            self.ryuapp.add_arp_table_entry(lvid, ip_address, mac_address)
        elif action == 'remove':
            self.ryuapp.del_arp_table_entry(lvid, ip_address)
    ...
    def _remove_tunnel_port(self, tun_ofport, tunnel_type):
        for remote_ip, ofport in self.tun_ofports[tunnel_type].items():
            if ofport == tun_ofport:
                self.int_br.check_in_port_delete_port(ofport)
